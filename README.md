...
- **Autonomous Skills Discovery & Execution ([skills.sh](https://skills.sh/) + Local Cache):** Built-in skill management (`skills_sh.py`) integrating the open agent skills ecosystem. Features autonomous pre-flight detection: the model automatically identifies when a task introduces a specialized framework or architecture (e.g. Supabase, Docker, Playwright, Next.js), downloads official vetted skill archives directly via GitHub codeload zip (bypassing git proxy and auth issues), and applies the instructions in `SKILL.md` immediately in the current turn. Zero overkill: routine programming tasks bypass discovery, and active MCP servers (such as runtime devtools) are strictly prioritized over downloading redundant skills.
- **Host Environment & Windows Utility Discovery:** Prompts and agent protocols instruct the model to discover native host tools (`where.exe`, `Get-Command`) before writing ad-hoc scripts. Automatically detects and leverages Windows tools like PuTTY (`plink.exe`, `putty.exe`) and OpenSSH (`ssh.exe`) for remote VPS and server operations.
...
- **Multimodal Vision Bridging (`input_image`):** Automatically extracts visual image payloads (e.g. from `take_screenshot` or MCP vision tools) out of tool outputs, strips raw inline base64 text to prevent context blowouts (~660k tokens -> ~500 visual tokens), and attaches them as native multimodal user messages accepted by llama.cpp.
- **Bidirectional Inlined Base64 Folding & Rehydration:** Automatically collapses massive inlined binary assets (data URIs and raw base64 >= 512 chars) in tool outputs into syntax-safe anchor stubs with SHA-256 digests and head/tail preservation, preventing tokenizer context blowouts. Seamlessly rehydrates the original binary bytes if the model references the anchor in `apply_patch` diffs or tool arguments, ensuring diffs apply cleanly to files on disk.
