...
def base_instructions(language: str) -> str:
    ...
    return (
        ...
        "dependencies, and architecture. SOURCE DISCOVERY GATE: when a code graph MCP namespace is attached, the first "
        "discovery call for source-code symbols or structure MUST be a matching graph tool; do not use rg, file readers, "
        "or helper scripts first. Use rg -n and rg --files only for literals, non-code files, or after the graph result "
        "is insufficient, and state the fallback reason briefly. After compaction, use the graph for only the missing "
        "symbol or range; do not reopen whole source files already captured in the checkpoint. Never claim graph MCP is "
        "unavailable without attempting its attached callable tool. Before creating helper scripts or downloading software, "
...
def proxy_environment(settings: Settings) -> dict[str, str]:
    values = {
        ...
        "CODEX_COMPACT_TASK_ANCHOR_MAX_CHARS": "8000",
        "CODEX_POST_COMPACT_OLD_USER_TOKEN_LIMIT": "0",
        "CODEX_POST_COMPACT_TOOL_OUTPUT_MAX_CHARS": "4000",
        "CODEX_POST_COMPACT_SOURCE_OUTPUT_MAX_CHARS": "12000",
        "CODEX_POST_COMPACT_SOURCE_TOTAL_CHARS": "60000",
        "CODEX_POST_COMPACT_PRUNE_TRIGGER_TOKENS": "116000",
        "CODEX_POST_COMPACT_TOOL_OUTPUT_KEEP_RECENT": "2",
        "CODEX_FORWARD_TOOL_PROGRESS": "1",
        "CODEX_PROGRESS_MAX_CHARS": "1200",
        "CODEX_FORCE_SERIAL_TOOL_CALLS": "1",
        "CODEX_UPSTREAM_RETRY_ATTEMPTS": "30",
        ...
        "CODEX_REPEAT_GUARD_THRESHOLD": "3",
